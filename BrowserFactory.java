...
public class BrowserFactory {
	...
	public void launchDriver(String browser, boolean useGrid) {
		...
		String mobileGridURL = "http//testing122334.seetest.com:4600/wd/hub";
		String accessKey ="eydhdygfffkdkdurhr37fhg86gjgu0khkh";
		...
		case "android":
			System.out.println("Starting Android");
			capabilities = DesiredCapabilities.android();
			capabilities.setCapability("accessKey", accessKey);
			capabilities.setCapability("deviceQuery", "@os='android' and @category='PHONE'");
			capabilities.setCapability("udid", "2789ef840");
			capabilities.setBrowserName(MobileBrowserType.CHROME);
			if (useGrid) {
				try {
					driver = new AndroidDriver<>(new URL(mobileGridURL), capabilities);
				} catch (MalformedURLException e) {
					System.out.println("Error in launching android mobile Chrome browser in Grid");
				}
			}
			
		case "ios":
			System.out.println("Starting iOS");
			...
			if (useGrid) {
				try {
					driver = new IOSDriver<>(new URL(mobileGridURL), capabilities);
				} catch (MalformedURLException e) {
					System.out.println("Error in launching ios mobile Chrome browser in Grid");
				}
			}
	...
	public WebDriver createChromeWithMobileEmulation(String deviceName, String browser, boolean useGrid) {
		String gridURL = "http://vsfghhhhd.com/wd/hub";
		if(System.getProperty("HUB_HOST") != null) {
			gridURL = "http://"+System.getProperty("HUB_HOST")+ ":4444/wd/hub";
		}
		...
		case "chrome":
			...
			if (useGrid) {
				try {
					...
					System.out.println("Error in launching browser in Grid");
				}
			}else {
				System.setProperty("webdriver.chrome.driver", ".\\drivers\\chromedriver.exe");
